...
class MyIterator
    ...
    public:
        ...
        MyIterator& operator++(){
            if(cur->rlink){
                //for(cur=cur->rlink;cur->llink;cur->llink);
                cur=cur->rlink;
                while(cur->llink){
                    cur=cur->llink;
                }
                ...
                if(cur->plink&&cur==cur->plink->llink){
                    ...
                    T1 v=cur->key;
                    // for(cur;cur->key<=v&&cur->plink!=nullptr;cur->plink){
...
public:
    ...
    void find_insert(treenode<T1,T2>* ctree, treenode<T1,T2>* ntree);
    void balance(treenode<T1,T2>* ntree);
    void print();
    void print(treenode<T1,T2>* iter);
    void print_n();
    
    treenode<T1,T2>* find(int a, treenode<T1,T2>* b);
...
template <typename T1,typename T2>
treenode<T1,T2>* RBT<T1,T2>::find(int a, treenode<T1,T2>* b){
    if(!b->k)return nullptr;
    if(a<b->k)return find(a,b->llink);
    else if(a==b->k)return b;
    else if(a>b->k)return find(a,b->rlink);
}
